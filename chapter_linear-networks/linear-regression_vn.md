...
Chúng ta sẽ thấy rằng việc truy cập đến toàn bộ dataset thường sẽ tiện hơn thông qua việc sử dụng *ma trận* $X$.
...
-->
Với một tập hợp điểm dữ liệu $\mathbf{X}$, sự dự đoán $\hat{\mathbf{y}}$ có thể được biểu diễn bằng phép nhân của matrix-vector:
...
Chính vì vậy, mặc dù chúng ta tự tin rằng mối quan hệ trong dữ liệu là tuyến tính nhưng chúng ta sẽ kết hợp với một số nhiễu để tạo ra kết quả tự nhiên hơn.
